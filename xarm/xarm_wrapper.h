...
namespace XArmWrapper {
	...
	extern "C" __declspec(dllexport) bool __stdcall is_connected(void);

	//typedef void(__stdcall* ConnectChangedCallback)(bool connected, bool reported);

	//extern "C" __declspec(dllexport) int __stdcall register_connect_changed_callback(ConnectChangedCallback Callback);
}
